    ...
    where T : SyncableEntity
{
    ...
    {
        var result = await js.InvokeAsync<JsonElement>("EntcuentroIndexedDb.getAll", _storeName);
        return Deserialize(result).Where(e => !e.IsDeleted);
    }

    public async Task<T?> GetByIdAsync(Guid id)
    {
        var result = await js.InvokeAsync<JsonElement?>("EntcuentroIndexedDb.getById", _storeName, id.ToString());
    ...
    public async Task SaveAsync(T entity)
    {
        ...
        await js.InvokeVoidAsync("EntcuentroIndexedDb.put", _storeName, json);
    }
    ...
    {
        var result = await js.InvokeAsync<JsonElement>("EntcuentroIndexedDb.getUnsynced", _storeName);
    ...
    public async Task MarkAsSyncedAsync(IEnumerable<Guid> ids)
    {
        ...
        await js.InvokeVoidAsync("EntcuentroIndexedDb.putBatch", _storeName, jsonArray);
    }
