...
namespace PlantillaDotNet.Web.Data;
    ...
    where T : SyncableEntity
{
    ...
    {
        var result = await js.InvokeAsync<JsonElement>("PlantillaDotNetIndexedDb.getAll", _storeName);
        return Deserialize(result).Where(e => !e.IsDeleted);
    }

    public async Task<T?> GetByIdAsync(Guid id)
    {
        var result = await js.InvokeAsync<JsonElement?>("PlantillaDotNetIndexedDb.getById", _storeName, id.ToString());
    ...
    public async Task SaveAsync(T entity)
    {
        ...
        await js.InvokeVoidAsync("PlantillaDotNetIndexedDb.put", _storeName, json);
    }
    ...
    {
        var result = await js.InvokeAsync<JsonElement>("PlantillaDotNetIndexedDb.getUnsynced", _storeName);
    ...
    public async Task MarkAsSyncedAsync(IEnumerable<Guid> ids)
    {
        ...
        await js.InvokeVoidAsync("PlantillaDotNetIndexedDb.putBatch", _storeName, jsonArray);
    }
