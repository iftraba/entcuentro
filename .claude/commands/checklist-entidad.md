Eres un asistente especializado en este proyecto PlantillaDotNet. Tu tarea es auditar una entidad existente y verificar que tiene todos los componentes necesarios correctamente implementados.
...
### 1. Modelo (`PlantillaDotNet.Shared/Models/{Entidad}.cs`)
- [ ] El archivo existe
...
### 2. DTOs (`PlantillaDotNet.Shared/DTOs/{Entidad}Dtos.cs`)
- [ ] El archivo existe
...
### 3. Interfaz de servicio (`PlantillaDotNet.Application/Interfaces/I{Entidad}Service.cs`)
- [ ] El archivo existe
...
### 4. Implementación (`PlantillaDotNet.Infrastructure/Services/{Entidad}Service.cs`)
- [ ] El archivo existe
...
### 5. DbContext (`PlantillaDotNet.Infrastructure/Data/AppDbContext.cs`)
- [ ] Tiene `public DbSet<{Entidad}> {Entidades} { get; set; }`

### 6. Registro DI (`PlantillaDotNet.Infrastructure/DependencyInjection.cs`)
- [ ] Tiene `services.AddScoped<I{Entidad}Service, {Entidad}Service>()`

### 7. Migración EF
- [ ] Existe un archivo de migración en `PlantillaDotNet.Infrastructure/Migrations/` que crea la tabla `{Entidades}`
- [ ] La migración está aplicada (verificar con `dotnet ef migrations list`)

### 8. Controlador API (`PlantillaDotNet.Api/Controllers/{Entidades}Controller.cs`)
- [ ] El archivo existe
...
### 11. UI
- [ ] Existe página de lista en `PlantillaDotNet.UI/Pages/`
- [ ] Existe página de formulario en `PlantillaDotNet.UI/Pages/`
